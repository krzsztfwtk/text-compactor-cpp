    ...
    if (colon_pos != std::string::npos) {
      ...
      if (key == "\"wordlist\"") {
        ...
        while (std::getline(config_file, line)) {
          ...
          int weight =
              std::stoi(line.substr(colon_pos + 1, comma_pos - colon_pos - 1));
...
int Parser::getCapitalNamesBoost() const { return capital_names_boost_; }

unsigned int Parser::getMinimumTFIDF() const { return minimum_tfidf_; }
